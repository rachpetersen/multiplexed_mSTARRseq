...
```{r}
unmeth_results<- read.table('/Users/rachelpetersen/Library/CloudStorage/GoogleDrive-rpetersen42@gmail.com/Other computers/My MacBook Pro (1)/Lea Lab/Human_mSTARRseq/model_results/model1_unmethonly_results.txt', header=T)
unmeth_results$windows <- rownames(unmeth_results)
colnames(unmeth_results)<- c("pval", "beta", "se", "id", "q", "info", "window")


meth_results<- read.table('/Users/rachelpetersen/Library/CloudStorage/GoogleDrive-rpetersen42@gmail.com/Other computers/My MacBook Pro (1)/Lea Lab/Human_mSTARRseq/model_results/model1_methonly_results.txt', header=T)
...
# Proportion of input fragment windows that fall into each annotation
```{r}
model1_allresults<-rbind(both_reg, both_notreg) %>%
  separate(window, c("chr", "start", "stop"))
model1_allresults$start<-as.numeric(model1_allresults$start)
...
SE4join <- left_join(model1_allresults, StrongEnhancer4, by=c("chr", "start", "stop")) %>%
  mutate(AP1 = ifelse(chromHMM == 0, 0, 1)) %>%
  dplyr::select(-chromHMM)
...
both<- merge(SE4join, SE5join)
both$strong_enhancer_any<-
```
