...
write.table(v_methonly$E,'normcounts_methonly.txt',row.names=F,sep='\t',quote=F)
```
...
write.table(fit_dna_df,'model1_methonly.txt',row.names=T,sep='\t',quote=F)
```
...
write.table(v_unmethonly$E,'normcounts_unmethonly.txt',row.names=F,sep='\t',quote=F)
```
...
fit_dna_df$info<-'not_regulatory'
fit_dna_df$info[which(fit_dna_df$id %in% fit_dna_keep01$id)]<-'regulatory'
write.table(fit_dna_df,'model1_unmethonly.txt',row.names=T,sep='\t',quote=F)
```
...
```{r}
methresults=read.table('model1_methonly.txt', row.names=1)
unmethresults=read.table('model1_unmethonly.txt', row.names=1)
```
...
lfsr$windows<- rownames(strong.site.locations)
pm$windows<- rownames(strong.site.locations)
...
write.table(lfsr_results,"mod2_mashr_results.txt")
```
