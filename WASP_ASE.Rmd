...
```{r}
#filter ASE total counts and ref counts matrices for sites that remain after WASP filtering.
WASPsites<-read.table("WASP_ASE_sites.txt")$x

totalcount<- read.table("ASE_totalcounts.txt")
totalcount_WASP<-subset(totalcount, site %in% WASPsites)

refcount<- read.table("ASE_refcounts.txt")
refcount_WASP<-subset(refcount, site %in% WASPsites)
...
```{r}
#filter for >0 counts in at least half of RNA replicates in 1 condition.
#filter for >0 counts in at least half of RNA replicates in 1 condition.
...
zerocounts_all <- Reduce(function(x, y) full_join(x, y, by = "site"), zerocounts_list) %>%
...
sites2remove<- rbind(closesiteslag, closesites)
sites2remove<- sites2remove[[1]]
...
###  filter methylated data  ###

## filter for sites that are in windows with some degree of regulatory activity in model 1 for methylated condition
```{r}
mod1_meth_results_posbeta<- read.table("./model_results/model1_methonly_results.txt") %>%
  filter(beta_meth > 0)

mod1_meth_results_posbeta$window<-rownames(mod1_meth_results_posbeta)

mod1_meth_results_posbeta_bed<-mod1_meth_results_posbeta %>%
  separate(window, c("chr", "start", "end")) %>%
  dplyr::select(chr, start, end)


#bedtools intersect totalcounts_keep_distfilter with mod1_meth_results_posbeta windows

filtered_methsites<-read.table("./WASP/ASE_methsites_filtered.bed") %>%
  mutate(site=paste(V1, V2, sep="_")) 
```
...
meth_notmono<- subset(Meth_ratio, meanRNAratio < 0.99 & meanRNAratio > 0.01); dim(meth_notmono) #2807


meth_mono<- subset(Meth_ratio, meanRNAratio > 0.99 | meanRNAratio < 0.01); dim(meth_mono) #3213
meth_mono$na_counts<-apply(subset(meth_mono[,1:5]),1,function(a) length(which(is.na(a))))
meth_mono_testable<- subset(meth_mono, na_counts <3); dim(meth_mono_testable) #2518 testable mono sites
write.table(meth_mono_testable, "./WASP/meth_monosites.txt", quote=F, row.names=F, col.names=F, sep="\t")
...
meth_notmono_testable<- subset(meth_notmono, na_counts <3); dim(meth_notmono_testable) #2,623 testable sites
...
###  filter unmethylated data  ###

## filter for sites that are in windows with some degree of regulatory activity in model 1 for unmethylated condition
```{r}
mod1_unmeth_results_posbeta<- read.table("./model_results/model1_unmethonly_results.txt") %>%
  filter(beta_unmeth > 0)

mod1_unmeth_results_posbeta$window<-rownames(mod1_unmeth_results_posbeta)

mod1_unmeth_results_posbeta_bed<-mod1_unmeth_results_posbeta %>%
  separate(window, c("chr", "start", "end")) %>%
  dplyr::select(chr, start, end)



totalcount_bed <- totalcount %>%
  separate(site, c("chr", "start")) %>%
  mutate(end= as.numeric(start) + 1) %>%
  dplyr:: select(chr, start, end)

#bedtools intersect totalcounts_keep_distfilter with mod1_unmeth_results_posbeta windows

filtered_unmethsites<-read.table("./WASP/ASE_unmethsites_filtered.bed") %>%
  mutate(site=paste(V1, V2, sep="_")) ; dim(filtered_unmethsites)
```
...
unmeth_notmono<- subset(Unmeth_ratio, meanRNAratio < 0.99 & meanRNAratio > 0.01); dim(unmeth_notmono) #4497

unmeth_mono<- subset(Unmeth_ratio, meanRNAratio > 0.99 | meanRNAratio < 0.01); dim(unmeth_mono) #3465 monoallelic sites
unmeth_mono$na_counts<-apply(subset(unmeth_mono[,1:6]),1,function(a) length(which(is.na(a))))
unmeth_mono_testable<- subset(unmeth_mono, na_counts <4);dim(unmeth_mono_testable) #3272 testable mono sites
write.table(unmeth_mono_testable, "./WASP/unmeth_monosites.txt", quote=F, row.names=F, col.names=F, sep="\t")
...
unmeth_notmono_testable<- subset(unmeth_notmono, na_counts <4);dim(unmeth_notmono_testable) #4,433 testable sites
...
##compare monoallelic sites identified in WASP versus the old pipeline
```{r}
new_mono_meth<- meth_mono_testable$site
old_mono_meth<- read.table("Nearly_mono_meth.txt", h=T)$site

new_mono_unmeth<- unmeth_mono_testable$site
old_mono_unmeth<- read.table("Nearly_mono_unmeth.txt", h=T)$site

mono_either_cond_old<-unique(c(old_mono_meth, old_mono_unmeth))
mono_either_cond_new<-unique(c(new_mono_meth, new_mono_unmeth))

retained_mono<- intersect(mono_either_cond_old, mono_either_cond_new)
(length(retained_mono)/ length(mono_either_cond_old)) *100 #86%
```
...
dim(fit_dna_keep) # 1,629 windows at q<0.01
dim(df) # 4433 total windows analyzed
nrow(fit_dna_keep)/nrow(df) #36.7%
df$info<-'not_significant'
df$info[which(df$id %in% fit_dna_keep$id)]<-'significant'
write.table(df, "./WASP/ASE_unmeth_results.txt", quote=F)
```
...
dim(fit_dna_keep) # 976 SNPs at q<0.01
dim(df) # 3037 total windows analyzed
nrow(fit_dna_keep)/nrow(df) #37.2% at q<0.01
df$info<-'not_significant'
df$info[which(df$id %in% fit_dna_keep$id)]<-'significant'
write.table(df, "./WASP/ASE_meth_results.txt")
```
...
tmp1<-unique(c(subset(match_ase_sign,meth_q<0.01)$site,subset(match_ase_sign,unmeth_q<0.01)$site)) #675 strong sites
...
# save output
write.table(get_lfsr(m2),'./WASP/ASE_mashr_LFSR.txt',row.names=F,sep='\t')
write.table(get_pm(m2),'./WASP/ASE_mashr_pm.txt',row.names=F,sep='\t')

lfsr=read.delim('./WASP/ASE_mashr_LFSR.txt')
pm=read.delim('./WASP/ASE_mashr_pm.txt')

# call MD activity with mashR output 
colnames(lfsr)<-colnames(pm)<-c('meth','unmeth')
lfsr$log2<-log2(abs(pm$unmeth/pm$meth))
pm$site<- lfsr$site<- strong.site.locations.mod4

#regulatory activity in at least one condition, log fold change of >1.5 between conditions
tmp1<-subset(lfsr,(meth<0.05 | unmeth<0.05) & (log2< -0.58 | log2>0.58)); dim(tmp1) #489 sites with MD activity >1.5LFC
...
write.table(lfsr_pm,"./WASP/ASE_mashr_results.txt")
```

#compare old and new results
```{r}
wasp_results<-read.table("./WASP/ASE_mashr_results.txt") %>%
  filter(info == "MD") %>%
  dplyr::select(site)

old_results<-read.table("./model_results/ASE_mashr_results.txt")%>%
  filter(info == "MD") %>%
  dplyr::select(site)
```

```{r}
in_both<- intersect(old_results$site, wasp_results$site)
```


##comparing ASE results
```{r}
wasp_meth<-read.table("./WASP/ASE_meth_results.txt") %>%
  filter(info == "significant") %>%
  dplyr::select(site) #976

old_meth<-read.table("./model_results/ASE_meth_results.txt") %>%
  filter(info == "significant") %>%
  dplyr::select(site) #1150

in_both<- intersect(old_meth$site, wasp_meth$site) #893

#893/1150
```

```{r}
wasp_unmeth<-read.table("./WASP/ASE_unmeth_results.txt") %>%
  filter(info == "significant") %>%
  dplyr::select(site) #1629

old_unmeth<-read.table("./model_results/ASE_unmeth_results.txt") %>%
  filter(info == "significant") %>%
  dplyr::select(site) #1858

in_both<- intersect(old_unmeth$site, wasp_unmeth$site) #1505
